...
*step below will list all styles available in your environment;
proc template;
	list styles;
run;

*Add ODS statement; 
*note that our ods excel statements will output with a specific style,
and have separate sheet names for each sheet generated. note that you need
additional 'ods excel options' statements for each sheet you want to rename.;
